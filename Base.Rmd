...
## 5·2 Choose k with multiple criteria ---------------------------------------
set.seed(42)

wss <- map_dbl(1:10, ~ kmeans(clus_vars, centers = .x, nstart = 25)$tot.withinss)
sil <- map_dbl(2:10, ~ silhouette(kmeans(clus_vars, .x, nstart = 25)$cluster,
                                  dist(clus_vars))[, "sil_width"] %>% mean())

par(mfrow = c(1, 2))
plot(1:10,  wss, type = "b", pch = 19, xlab = "k", ylab = "Total WSS",
     main = "Elbow")
abline(v = 3, lty = 2, col = 2)
plot(2:10, sil, type = "b", pch = 19, xlab = "k", ylab = "Avg. Silhouette",
     main = "Silhouette")
abline(v = which.max(sil) + 1, lty = 2, col = 2)
par(mfrow = c(1, 1))

k_opt <- 3  # both criteria agree

## 5·3 Fit & visualise clusters ----------------------------------------------
cols   <- c("1" = "#0080FF",   # blue
            "2" = "#FADA5E",   # yellow
            "3" = "#A8A8A8")   # grey
shapes <- c("1" = 16,  # ● Solid performers
            "2" = 17,  # ▲ Social butterflies
            "3" = 15)  # ■ At-risk cohort
fviz_cluster(
  km3, data = clus_vars,
  palette      = cols,
  ...
  theme(
    legend.position   = c(0.92, 0.80),   # adjust to taste
...
cluster_summary <- stu %>% 
  ...
  summarise(
    ...
    GoOut       = mean(goout),
    WorkdayAlc  = mean(dalc),
    WeekendAlc  = mean(walc),
    .groups = "drop"
  )

interpret <- tribble(
  ~cluster, ~Behaviour,                                   ~`Risk Profile`,
  1,        "Moderate studytime, few failures",           "Solid performers",
  2,        "Lowest studytime, highest social drinking",  "Social butterflies",
  3,        "High failures & absences",                   "At-risk cohort"
...
cluster_summary %>% 
  ...
  tab_header(title = "Cluster Summary & Interpretation")
```
...
## 6·3 Performance ------------------------------------------------------------
prob_pred  <- predict(log_fit, testL, type = "prob")   # .pred_yes / .pred_no
class_pred <- predict(log_fit, testL, type = "class")  # .pred_class

log_pred <- testL %>%
  select(pass) %>%            # truth column
...
metrics(
  log_pred,
  truth    = pass,
  estimate = .pred_class,   # <- factor for accuracy()
  .pred_yes                 # <- numeric for roc_auc()
)
...
conf_mat(class_pred, truth = pass, estimate = .pred_class)

## 6·4 Tidy coefficients with odds ratios ------------------------------------
