...
# CH is only defined for k ≥ 2
ch <- map_dbl(2:10, function(k) {
  km  <- kmeans(clus_vars, centers = k, nstart = 25)
  cs  <- cluster.stats(dist(clus_vars), km$cluster)
  cs$ch                       # Calinski–Harabasz value
})

## 2·4 Plot the three indices together ----------------------------------------
par(mfrow = c(1, 3))
...
plot(2:10, ch,  type = "b", pch = 19,
     xlab = "k", ylab = "Calinski–Harabasz",
     main = "Calinski–Harabasz index")
abline(v = which.max(ch) + 1, lty = 2, col = 2)
